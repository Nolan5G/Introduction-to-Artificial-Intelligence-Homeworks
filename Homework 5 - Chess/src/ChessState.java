...
class ChessState {
	...
	/// ended the game.
	boolean move(ChessState.ChessMove move) {
	...
	public static int convertColumnAlphaToColumnNumber(String str) {
		if(str.length() != 1) {
			throw new RuntimeException("Cannot convert column " + str + " to a number.  Size is not length 1");
		}
		if(str.equalsIgnoreCase("A")) { return 1; }
		else if(str.equalsIgnoreCase("B")) { return 2; }
		else if(str.equalsIgnoreCase("C")) { return 3; }
		else if(str.equalsIgnoreCase("D")) { return 4; }
		else if(str.equalsIgnoreCase("E")) { return 5; }
		else if(str.equalsIgnoreCase("F")) { return 6; }
		else if(str.equalsIgnoreCase("G")) { return 7; }
		else if(str.equalsIgnoreCase("H")) { return 8; }
		else {
			throw new RuntimeException("Cannot convert column " + str + "to a number. Letter is not from [A-H]");
		}
	}
	
	public static String convertMoveToAlphanumeric(ChessState.ChessMove m) throws NullPointerException {
		if(m == null) { 
			throw new NullPointerException(); 
		}
		return convertColumnIntToString(m.xSource)
				+ (m.ySource + 1)
				+ convertColumnIntToString(m.xDest)
				+ (m.yDest + 1);
	}
		
	private static String convertColumnIntToString(int col) {
		if(col == 0) { return "A"; }
		else if(col == 1) { return "B"; }
		else if(col == 2) { return "C"; }
		else if(col == 3) { return "D"; }
		else if(col == 4) { return "E"; }
		else if(col == 5) { return "F"; }
		else if(col == 6) { return "G"; }
		else if(col == 7) { return "H"; }
		else { return null; }
 	}
	...
	static class ChessMove {
		...
		public ChessMove(int a, int b, int c, int d) {
			xSource = a;
			ySource = b;
			xDest = c;
			yDest = d;
